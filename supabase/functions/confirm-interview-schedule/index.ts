...
const getDbDepartment = (jobTitle)=>{
...
// 헬퍼 함수: 부서별 면접관 조회
const getInterviewersByDepartment = async (supabase, department)=>{
  const dbDepartment = getDbDepartment(department); // ✨ 헬퍼 함수 사용
  const { data: users, error } = await supabase.from('users').select('id, name, email, role, department').eq('department', dbDepartment).in('role', [
    'manager',
    'viewer'
  ]);
...
const getGoogleAccessToken = async (supabase)=>{
  console.log('[AUTH] 채용 담당자의 Refresh Token 조회 시작...');
  // 특정 채용 담당자 계정의 Refresh Token을 사용 (하드코딩)
  const { data: recruiter, error: recruiterError } = await supabase.from('users').select('provider_refresh_token').eq('email', 'recruiter.dayeon@gmail.com').single();
...
const sendSlackNotification = async (jobTitle, applicantName, startTime, endTime, applicationId)=>{
  // @ts-ignore
        ...
        elements: [
          {
            ...
            url: `${Deno.env.get('FRONTEND_URL') || 'https://ats-recruiter.vercel.app'}/application/${applicationId}`
          }
        ]
      }
    ]
...
serve(async (req)=>{
  ...
  try {
    ...
    Deno.env.get('SUPABASE_URL') ?? '', // @ts-ignore
    Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '');
    // ✨ [수정] 테이블에 직접 INSERT하는 대신, 중복을 확인하는 DB 함수를 호출합니다.
    const { data: bookingResult, error: rpcError } = await supabase.rpc('book_interview_slot', {
      p_application_id: applicationId,
      p_job_id: jobId,
      p_department: getDbDepartment(department),
    ...
    // --- 2. 필요한 정보 조회 ---
    const { data: appData, error: appError } = await supabase.from('applications').select('name, email, jobs(title)').eq('id', applicationId).single();
    ...
    const event = {
      summary: `${appData.jobs.title} 면접: ${appData.name}님`,
      description: `지원자 상세 페이지: ${Deno.env.get('FRONTEND_URL') || 'https://ats-recruiter.vercel.app'}/application/${applicationId}`,
      start: {
        dateTime: startTime,
        timeZone: 'Asia/Seoul'
      },
    ...
    if (resendApiKey) {
      const formattedStartTime = new Date(startTime).toLocaleString('ko-KR', {
        timeZone: 'Asia/Seoul',
        dateStyle: 'full',
        timeStyle: 'short'
      });
      const emailContent = `<h4>🎉 면접 일정이 확정되었습니다.</h4><p>안녕하세요, ${appData.name}님.</p><p>${appData.jobs.title} 포지션 면접이 아래와 같이 확정되었습니다.</p><p><strong>일시:</strong> ${formattedStartTime}</p><p>화상회의 링크가 포함된 캘린더 초대를 확인해주세요.</p>`;
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          subject: `[DATS] ${appData.jobs.title} 면접 일정이 확정되었습니다.`,
          html: emailContent
        })
      });
    }
    // --- 5. Slack 알림 발송 ---
    await sendSlackNotification(appData.jobs.title, appData.name, startTime, endTime, applicationId);
    // --- 6. 지원서 상태 업데이트 ---
