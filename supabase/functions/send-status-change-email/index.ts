...
// 헬퍼 함수 추가: 토큰 생성
const generateToken = (applicationId, secret)=>{
...
const getCalendarBusyTimes = async (calendarId, startDate, endDate, addLog)=>{
  try {
    ...
    const jwt = await new jose.SignJWT({
      ...
      alg: 'RS256',
      typ: 'JWT'
    }).setIssuedAt().setIssuer(serviceAccount.client_email).setAudience('https://oauth2.googleapis.com/token').setExpirationTime('1h').sign(privateKeyObject);
...
const generateAvailableSlots = (interviewersBusyTimes, dateRange, timeRange, duration, addLog)=>{
  const slots = [];
  try {
...
serve(async (req)=>{
  ...
  try {
    const { applicantName, applicantEmail, jobTitle, company, newStatus, applicationId, interviewDetails } = await req.json();
    addLog('🚀 === 상태 변경 이메일 발송 요청 시작 ===');
    addLog(`📊 요청 데이터: ${JSON.stringify({
      applicantName,
      applicantEmail,
      jobTitle,
      company,
    ...
    switch(newStatus){
      case 'interview':
        ...
        if (interviewers.length === 0) {
          ...
          const allBusyTimes = []; // For logging
          ...
          const availableSlots = generateAvailableSlots(interviewersBusyTimes, {
            start: interviewSettings.date_range_start,
            end: interviewSettings.date_range_end
          }, {
            start: interviewSettings.time_range_start,
            end: interviewSettings.time_range_end
          }, interviewSettings.duration, addLog);
          ...
          if (saveResult) {
            ...
            // DB 저장 확인
            const { data: savedSlots, error: checkError } = await supabase.from('interview_available_slots').select('*').eq('application_id', applicationId).eq('is_available', true);
            if (checkError) {
    ...
    return new Response(JSON.stringify({
      ...
      logs: [
        ...logs,
        `[ERROR] 전체 프로세스 중 오류: ${error.message}`
      ]
    }), {
      headers: {
        ...corsHeaders,
